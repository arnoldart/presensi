...
def add_user():
    ...
    if not all(key in data and data[key] for key in ['nim', 'password', 'username']):
        return jsonify({'error': 'Username, password, and nim cannot be empty'}), 400

    nim, nama, password = data['nim'], data['username'], data['password']
    hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
...
def add_new_presensi():
    try:
        ...
        return jsonify({'message': 'Presensi added successfully', 'id_presensi_kelas': presensi.id}), 201
...
def get_users():
    try:
        ...
        user_data = [{'id': user.id, 'nim': user.nim, 'name': user.nama} for user in users]
